...
STOP_THRESHOLD_DISTANCE = 6  # meters
...
class DecisionMaker(object):
    def __init__(self, config, publishing_rate):
        ...
        self.targets = [
            (26.17592989, -47.50193696,   1.01084051, 5),
            (59.25447372, -47.2042948,    0.996886, 5),
            (68.74508458, -42.61156511,   1.03150283, 5),
            (74.53964479, -25.98481017,   1.21311468, 5),
            (0, 0, 0, 0),
        ]
    ...
    def init_overtaking(self, preceding_object):
        ...
        overtaking_target_points = np.array([
                ...
                [overtaken_x - self.lane_width, overtaken_y + CAR_LENGHT * 2, overtaken_z],
                [overtaken_x, overtaken_y + CAR_LENGHT * 4, overtaken_z],
        ])
        rospy.loginfo(f"reach (car ref) {overtaking_target_points}")
        ...
        rospy.loginfo(f"reach {overtaking_target_points}")


    def get_direction(self, current_position):
        # reach last target point -> finish overtaking
        if self.current_target_point is None:
            tmp = self.targets.pop(0)
            self.current_target_point = [tmp[0], tmp[1], tmp[2]]
            self.current_target_speed = tmp[3]
        ...
        rospy.loginfo(f"distance_to_target={distance_to_target}")
        ...
        if distance_to_target < 1:
            ...
            self.last_distance_to_target = None
            self.last_distance_to_target_time = None
            rospy.loginfo( f"new target (car ref) {self.map_handler.get_car_world_position(self.current_target_point)}")
            rospy.loginfo( f"new target (world ref) {self.current_target_point}")

        target = self.map_handler.get_car_world_position(self.current_target_point)

        # rospy.loginfo(f"reach (car ref): {target}")
        return [DECISION_STATE.REACH_TARGET], target, speed
    ...
    def decision_maker(self):
        # HANDLE PANIC MODE
        if self.panic_mode:
            self.controller.handle_decision([DECISION_STATE.PANIC_MODE])
            return
        
        # NO MORE DIRECTION → SCENRIO IS DONE
        if len(self.targets) == 0:
            self.controller.handle_decision([DECISION_STATE.DONE])
            return
        ...
        if self.overtaking:
            if self.target_reached_count > self.overtaking_end_step:
                rospy.loginfo("ACTION: overtaking ended")
                self.overtaking = False
                self.activate_camera(forward_right=False)

        # GET DIRECTION
        state, target_position, target_speed = self.get_direction(current_position)
        current_state.extend(state)
        # rospy.loginfo(f'target_speed: {target_speed}')
        
        self.controller.handle_decision(current_state, target_position, self.real_speed, target_speed)
