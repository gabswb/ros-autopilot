...
class DistanceExtractor (object):
	...
	def convert_pointcloud(self):
		...
		temp_points = projected_points.T.reshape(-1, 3)
		norms = np.linalg.norm(temp_points, axis=1)
		ro = norms.reshape(1, -1)

		#ro = np.linalg.norm(projected_points)

		# rospy.loginfo(f"projected points : {projected_points.shape}")
		# rospy.loginfo(f"ro : {ro.shape}")
		# #rospy.loginfo(f"dist  : {self.distortion_parameters.shape}")
		# rospy.loginfo(f"ro*distortion_parameters : {(ro*self.distortion_parameters).shape}")
		# rospy.loginfo(f"deno : {(projected_points[2]+(ro*self.distortion_parameters)).shape}")

		# rospy.loginfo(f" ro {ro}")
		# rospy.loginfo(f" z {projected_points[2]}")
		# rospy.loginfo(f" total {projected_points[2]+(ro*self.distortion_parameters)}")
		...
		image_points = np.asarray((
			projected_points[0] / (projected_points[2]+(ro*self.distortion_parameters)),
			projected_points[1] / (projected_points[2]+(ro*self.distortion_parameters)),
		)).transpose().reshape(-1, 2)
	
		image_points2 = np.asarray((
			projected_points[0] / (projected_points[2]),
			projected_points[1] / (projected_points[2]),
		)).transpose()

		rospy.loginfo(f"image_points : {image_points}")
		rospy.loginfo(f"image_points2 : {image_points2}")
